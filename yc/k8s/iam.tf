...
resource "yandex_resourcemanager_folder_iam_member" "k8s-role-different-network0" {
  count     = var.is_network_different_folder && !local.create_sa ? 1 : 0
  folder_id = var.different_folder_id
  role      = "vpc.privateAdmin"
  member    = "serviceAccount:${yandex_iam_service_account.master[0].id}"
}
resource "yandex_resourcemanager_folder_iam_member" "k8s-role-different-network1" {
  count     = var.is_network_different_folder && !local.create_sa ? 1 : 0
  folder_id = var.different_folder_id
  role      = "vpc.user"
  member    = "serviceAccount:${yandex_iam_service_account.master[0].id}"
}
resource "yandex_resourcemanager_folder_iam_member" "k8s-role-different-network2" {
  count     = var.is_network_different_folder && !local.create_sa ? 1 : 0
  folder_id = var.different_folder_id
...
resource "yandex_resourcemanager_folder_iam_member" "node_account_different_folder" {
  count     = var.is_network_different_folder && !local.create_sa ? 1 : 0
  folder_id = var.different_folder_id
  role      = "container-registry.images.puller"
  member    = "serviceAccount:${yandex_iam_service_account.node_account[0].id}"
